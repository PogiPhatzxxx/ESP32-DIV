...
void displayLogo(uint16_t color, int displayTime) {
  ...
  tft.fillRect(logoX, logoY, bitmapWidth, bitmapHeight, TFT_BLACK);
  tft.drawBitmap(logoX, logoY, bitmap_icon_pogi, bitmapWidth, bitmapHeight, color);
...
void runUI() {
    ...
    if (millis() - lastTouchCheck >= touchCheckInterval) {
        if (ts.touched() && feature_active) { 
            ...
            int x = ::map(p.x, 3800, 239, 0, SCREEN_WIDTH - 1);
            int y = ::map(p.y, 3800, 319, 0, SCREENHEIGHT - 1);
